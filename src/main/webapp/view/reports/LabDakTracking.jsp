		...
		<div class="row">
		 <form action="DakTrackingPrint.htm" target="_blank">
			<div class="<% if (RedirFromCommon == null || RedirFromCommon.isEmpty()) { %>col-md-5<%}else{ %>col-md-7<%} %> heading-breadcrumb">
				<h5 style="font-weight: 700 !important;width:600px!important;">Lab DAK TRACKING&nbsp;&nbsp;&nbsp;<%if(dakTrackingData!=null && dakTrackingData.size()>0){ %><span style="font-size: 15px;">(
			...
			<div class="card-body" style="margin-top:-40px;"> 
 				...
 				for(Object[] obj:dakTrackingData){ 
 					...
 					if (obj[4] != null) { 
 			            ...
 			            if (obj[6] != null) {
 			                InitiatedDate = LocalDate.parse(obj[6].toString().split("T")[0]);
 			            }

 			            if (obj[7] != null) {
 			                DistributedDate = LocalDate.parse(obj[7].toString().split("T")[0]);
 			            }

 			            if (obj[17] != null) {
 			                AckDate = LocalDate.parse(obj[17].toString().split("T")[0]);
 			            }

 			            if (obj[18] != null) {
 			                ReplyDate = LocalDate.parse(obj[18].toString().split("T")[0]);
 			            }

 			            if (obj[12] != null) {
 			                PandCDate = LocalDate.parse(obj[12].toString().split("T")[0]);
 			            }

 			            if (obj[16] != null) {
 			                ApprovedDate = LocalDate.parse(obj[16].toString().split("T")[0]);
 			            }

 			            if (obj[14] != null) {
 			                ClosedDate = LocalDate.parse(obj[14].toString().split("T")[0]);
 			            }
 			            
 			            if (obj[23] != null) {
 			            	ForwardedDate = LocalDate.parse(obj[23].toString().split("T")[0]);
 			            }
 			...
 			<input type="hidden" name="viewfrom" value="DakTracking">
					 <div class="timeline-row">
						...
						<h6 style="background-color:#007bff;"> DAK Created By &nbsp;/&nbsp;<span id="InitiatedByFullName"></span>
...
<!--Forward is only for p&cDO and that to if and only if director approval is required  -->
	...
	<%}%>
				...
				<span style="font-size: 18px;"><b>Note : DAK Created And Still Not Distributed From <%=DistributedNotificationShowingPlace %> Days </b></span>
				<%} %>
...
if(InitiatedDetails!=null){
	...
	$.ajax({
		type : "GET",
		url : '<%=url%>'+"GetInitiatedByFullName.htm",	
		datatype : 'json',
		data : {
...
console.log('DakDetails',DakDetails);
if(DakDetails!=null){
	var DakId= DakDetails.value;
	console.log('DakId46346436436346346346',DakId);
	$.ajax({
		type : "GET",
		url : '<%=url%>'+"GetAcknowledgedMembers.htm",	
		datatype : 'json',
		data : {
			dakId : DakId
		},
...
if(DakDetails!=null){
	...
	console.log('SelDakId',SelDakId);
	$.ajax({
		type : "GET",
		url : '<%=url%>'+"GetAcknowledgeMembers.htm",	
		datatype : 'json',
		data : {
			SelDakId : SelDakId
		},
...
function OpenMoreDetails(data)
{
    ...
    var url1 = '<%=url%>'+"GetAcknowledgedMembers.htm";
    var url2 = '<%=url%>'+"GetRepliedMemberName.htm";
...
if(DakDetails!=null){
	...
	console.log('dakId6346534',dakId);
	$.ajax({
		type : "GET",
		url : '<%=url%>'+"GetRepliedMemberName.htm",	  
		datatype : 'json',
		data : {
			dakId : dakId
		},
...
if(DakDetails!=null){
	...
	$.ajax({
		type : "GET",
		url : '<%=url%>'+"GetReplyMemberName.htm",	  
		datatype : 'json',
		data : {
...
if(RepliedDetails!=null){
	...
	$.ajax({
		type : "GET",
		url : '<%=url%>'+"RepliedByPAndCFullName.htm",	
		datatype : 'json',
		data : {
...
if(ApprovedDetails!=null){
	...
	$.ajax({
		type : "GET",
		url : '<%=url%>'+"ApprovedByFullName.htm",	
		datatype : 'json',
		data : {
...
if(ClosedDetails!=null){
	...
	console.log('<%=url%>'+"ClosedByFullName.htm");
	$.ajax({
		type : "GET",
		url : '<%=url%>'+"ClosedByFullName.htm",	
		datatype : 'json',
		data : {
			ClosedBy : ClosedBy
		},
